...
public class BST<T> {
    ...
    public BST(T[] array, Comparator<T> cmp) throws IllegalArgumentException {
        ...
        if (!isSorted(array, cmp)) {
            throw new IllegalArgumentException("BST(): Array argument unsorted. Cannot construct BST.");
        }
    ...
    private Node remove(T data, Node node, Comparator<T> cmp) {
        ...
        4. Otherwise,
            ...
            4. Otherwise
                ...
                passing it node's rightchild and the minimum data of node's right subtree 
    ...
    public T sharedPrecursor(T data1, T data2, Comparator<T> cmp) throws IllegalArgumentException {
        if ((search(data1, cmp) == null) || (search(data2, cmp) == null)) {
            throw new IllegalArgumentException("Either data1 or data2 don't exist (in BST).");
        }
    ...
    private T sharedPrecursor(T data1, T data2, Node currLevel, Comparator<T> cmp) {
        // Keep going until currLevel is between data1 and data2
        // if ((cmp.compare(currLevel.data, data1) > 0) && (cmp.compare(currLevel.data, data2) < 0)) {
        //     return currLevel.data;
        // }

        // If currLevel.data is more than both
        if ((cmp.compare(currLevel.data, data1) > 0) && (cmp.compare(currLevel.data, data2) > 0)) {
            return sharedPrecursor(data1, data2, currLevel.left, cmp);
        }
        // if currLevel.data is less than both
        else if ((cmp.compare(currLevel.data, data1) < 0) && (cmp.compare(currLevel.data, data2) < 0)) {
            return sharedPrecursor(data1, data2, currLevel.right, cmp);
        }
        // if currLevel.data is in between
        else {
            return currLevel.data;
        }
    }
